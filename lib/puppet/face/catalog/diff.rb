...
Puppet::Face.define(:catalog, '0.0.1') do
  ...
  action :diff do
    ...
    when_invoked do |catalog1, catalog2, options|
      ...
      nodes[:fact_search]        = options[:fact_search]
      nodes
    end
    ...
    when_rendering :console do |nodes|
      ...
      next if node == :total_percentage or node == :total_nodes or node == :most_changed or node == :with_changes or node == :most_differences or node == :pull_output or node == :date or node == :fact_search
